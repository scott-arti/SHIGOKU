...
### 3.4 `report export-targets`

整合性が取れた report から single-session structured target bundle を生成します。

```bash
.venv/bin/shigoku-ops --json report export-targets \
  --report <haddix_report.md> \
...
| オプション | 説明 |
| :--- | :--- |
| `--report` | 対象 report。`consistent` の時だけ続行 |
| `--session` / `--sessions-dir` | source session 解決の補助 |
| `--output-dir` | `attack_targets.json` / `endpoints.{json,csv,md}` の出力先 |
| `--ttl-days` | manifest の TTL |
| `--max-records` | エクスポート件数上限 |
| `--overwrite` | 既存 artifact の上書き |

補足:
- `attack_targets.json` は再投入用の machine-readable 正本です。
- `attack_targets.json` には `generated_at` / `ttl_days` / `scope_snapshot` / source provenance が入り、期限切れまたは provenance 欠落の bundle は入力側で fail-closed になります。
- `endpoints.{json,csv,md}` は人間向け表示用で、query token や secret らしき値は redaction されます。

### 3.5 `report findings`

整合性が取れた report の source session から canonical findings を取得します。

```bash
.venv/bin/shigoku-ops --json report findings \
  --report <haddix_report.md> \
  --finding-preset triage
```
...
| オプション | 説明 |
| :--- | :--- |
| `--detection-class` | detection class で絞り込み |
| `--max-findings` | 件数上限 |
...
整合性が取れた report の source session から抽出済みエンドポイント一覧を返します。

```bash
.venv/bin/shigoku-ops --json report endpoints \
  --report <haddix_report.md> \
  --host 127.0.0.1
```
...
### 4.2 `session export-targets`

single-session structured target bundle を直接生成します。

```bash
.venv/bin/shigoku-ops --json session export-targets \
  --session <session.json> \
  --output-dir workspace/out
```
...
session から抽出済みエンドポイント一覧を返します。

```bash
.venv/bin/shigoku-ops --json session endpoints \
  --session <session.json> \
  --method GET
```
...
### 4.4 `session resolve-from-report`
...
## 5. `shigoku-ops findings`

cross-session の FindingsRepository を扱います。

### 5.1 `findings list` / `search` / `stats`

```bash
.venv/bin/shigoku-ops --json findings list --db-path ~/.shigoku/findings.db --limit 20
.venv/bin/shigoku-ops --json findings search --db-path ~/.shigoku/findings.db --target example.com --verified-only
.venv/bin/shigoku-ops --json findings stats --db-path ~/.shigoku/findings.db
```
...
| アクション | 主な用途 | 主要オプション |
| :--- | :--- | :--- |
| `list` | 保存済み finding の一覧 | `--limit`, `--offset`, `--order-by`, `--asc` |
| `search` | server-side filter つき検索 | `--severity`, `--vuln-type`, `--target`, `--source-agent`, `--verified-only`, `--limit` |
| `stats` | 集計 | `--db-path` |

### 5.2 `findings export-targets`

FindingsRepository から cross-session structured target bundle を生成します。

```bash
.venv/bin/shigoku-ops --json findings export-targets \
  --db-path ~/.shigoku/findings.db \
  --target example.com \
  --output-dir workspace/out
```
...
| オプション | 説明 |
| :--- | :--- |
| `--allowed-host` | 明示スコープ host。mixed-scope export を許す時の正本 |
| `--target` | 検索条件兼スコープ拘束の補助 |
| `--verified-only` | verified finding のみに限定 |
| `--max-records` | エクスポート件数上限 |
| `--output-dir` | `attack_targets.json` / `endpoints.{json,csv,md}` の出力先 |
| `--overwrite` | 既存 artifact の上書き |

補足:
- mixed-scope の findings DB に対して `--allowed-host` または `--target` がない場合は `cross_session_scope_required` で fail-closed します。
- `attack_targets.json` は再投入用の machine-readable 正本です。
- `attack_targets.json` には `generated_at` / `ttl_days` / `scope_snapshot` / source provenance が入り、期限切れまたは provenance 欠落の bundle は入力側で fail-closed になります。
- `endpoints.{json,csv,md}` は人間向け表示用で、query token や secret らしき値は redaction されます。

## 6. `shigoku-ops validate`
...
## 9. `shigoku-ops runtime-control`

### 9.1 `runtime-control gate`
...
自然言語のオペレータ指示を allowlist 済み command preview へ変換し、必要なら実行します。

```bash
.venv/bin/shigoku-ops --json ops intent \
  --intent "このレポートから API だけ Fuzz して" \
  --report <haddix_report.md> \
...
| :--- | :--- |
| `--intent` | 自然言語指示 |
| `--report` / `--session` / `--attack-targets` | source context |
| `--target` | `main.*` 実行時の target |
| `--wordlist` | attack intent 用のカスタム wordlist |
| `--execute` | preview 後に実行 |
| `--approve` | TTY 確認をスキップ |
| `--main-dry-run` | `main.*` step に `--dry-run` を付与して限定実行 |

補足:
- heuristic で解けない `malformed intent` は `intent_unresolved` で止まります。
- LLM fallback が利用不能でも `intent_llm_unavailable` を返して fail-closed し、CLI 全体はクラッシュさせません。
...
- `session.findings`
- `session.export-targets`
- `main.attack-targets`
- `main.recon-resume`

外部エージェント向け function-calling schema 例:

```json
{
  "name": "shigoku_ops_intent",
  "description": "Translate a natural-language SHIGOKU operator request into a preview or limited execution flow.",
  "parameters": {
    "type": "object",
    "properties": {
      "intent": { "type": "string" },
      ...
      "report": { "type": "string" },
      "session": { "type": "string" },
      "attack_targets": { "type": "string" },
      "wordlist": { "type": "string" },
      "execute": { "type": "boolean" },
      "approve": { "type": "boolean" },
      "main_dry_run": { "type": "boolean" }
    },
    "required": ["intent"]
  }
}
...
推奨運用:
1. まず `--execute` なしで preview を確認する。
2. attack 系は `--execute --approve --main-dry-run` で限定実行を先に試す。
3. `0326` の artifact を使う場合は `report export-targets` または `findings export-targets` の出力 `attack_targets.json` を `--attack-targets` へ渡す。

停止条件 / ロールアウト制御:
- `config/shigoku.yaml` の `ops_intent.feature_flag=false` で導線を止め、`ops_intent.kill_switch=true` で即時 fail-closed にする。
- `ops_intent.daily_llm_budget` 到達、または `intent_unresolved` / `intent_llm_unavailable` / `command_timeout` / `report_consistency_inconsistent` / `allowed_hosts_mismatch` が連続する場合は preview-only 運用へ戻す。
- attack 系は non-TTY で自動承認せず、preview または dry-run に戻して原因を解消してから再開する。

| アクション | 主な用途 | 主要オプション |
| :--- | :--- | :--- |
| `intent` | NL→allowlist preview / optional execution | `--intent`, `--report`, `--session`, `--attack-targets`, `--execute`, `--approve`, `--main-dry-run` |
...
### 12.7 HITL / deferred / report replay
