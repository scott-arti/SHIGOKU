...
| 帯域外(OOB)検出基盤 / blind XXE | ブラインド（in-band 反射なし）を外向きコールバックで確定 | `detection/oob_provider`（新設） + `utils/oob_listener`(LocalOOBListener) + `injection/smart_blind_xxe`（新設） | ◎ **OOB 確認基盤を整備し blind XXE を実対象で完全3ゲート確定**（旧 `oob_correlator.py` は poll/start_server が Placeholder の死んだ scaffolding だったため削除し、`OOBProvider` 抽象化＋`LocalOOBProvider`（自前 HTTP 受信・依存ゼロ）に統一。受信器は差し替え可能で、将来は自前ホスト interactsh（DNS＋HTTP・公開）を無改修で差し込む設計＝エンジンは宛先しか見ない。**実 SKF xxe ラボ**（`--network host` でラボ→受信器到達を確保）で、外部実体 `SYSTEM "http://<受信器>/callback/<token>"` を送ると標的の XML パーサが我々の受信器を fetch し**一意 token が標的から到達**（in-band は空＝ブラインド）。確定バーに**vuln_type 横断の汎用 OOB マーカー `oob_interaction_received`**（token 非空＋token が payload に実在＋interaction_received＋受信 interaction.path に token 実在。乱数 token で偶然混入・捏造不可）を追加→再現チェッカーが**新 token＋新コールバックで再送し受信器で再観測**→matched→CONFIRMED、かつ**実 poc_judge 5/5 承認**（初回は OOB を `#` コメント要約で書き 0/5→**標的が受信器に送ってきた生の inbound リクエスト＋token 相関**を raw 提示して 5/5＝poc-judge-raw-evidence の OOB 版）。SGK-2026-0494。token 未到達/payload に token 無しは fail-closed）。非破壊（良性コールバックのみ）。**この基盤で blind SSRF / OOB SQLi / デシリアライズも同型で確定可能に**（HTTP OOB。DNS-only は DNS 受信器プロバイダで別途）。ローカルではラボ→受信器の到達性に工夫が要り、実インターネット標的には公開受信器（自前 interactsh）が必要＝別タスク |
...
| 安全でないデシリアライズ | ◎ | 低(L1) | 今セッション新設。OOB 基盤横展開で pickle デシリアライズ(RCE級)を◎。Python pickle 単一形態・SKF 単一・Java/PHP builder と in-band 確認は別途 |
