"""Character tests for API probe helper functions after extraction."""
...
def test_extract_mass_assignment_excluded_params_skipped():
    bodies = ['{"url": "test", "method": "GET", "params": {}}']
    result = extract_mass_assignment_schema_candidates(
        response_bodies=bodies, excluded_params=EXCLUDED,
    )
...
def test_build_mass_assignment_variant_bool_flips():
...
def test_build_mass_assignment_variant_int_increments():
    probe = {"limit": 10}
    result = build_mass_assignment_variant_payload(probe, "marker")
    assert result["limit"] == 11


def test_build_mass_assignment_variant_str_admin_flips():
    probe = {"role": "admin"}
    result = build_mass_assignment_variant_payload(probe, "marker")
    assert result["role"] == "auditor"


def test_build_mass_assignment_variant_list_mutates():
    probe = {"roles": ["user"]}
    result = build_mass_assignment_variant_payload(probe, "marker")
    assert result["roles"] == ["admin"]


def test_build_mass_assignment_variant_skips_probe_key():
...
def test_build_mass_assignment_variant_empty_list():
    probe = {"roles": []}
    result = build_mass_assignment_variant_payload(probe, "marker")
    assert result["roles"] == ["auditor"]


def test_build_mass_assignment_variant_other_type_preserved():
    probe = {"unknown": None}
    result = build_mass_assignment_variant_payload(probe, "marker")
