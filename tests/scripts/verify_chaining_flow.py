...
async def verify_chaining_flow():
    ...
    # pytest 実行時の loop race を避けるため、この検証専用の EventBus を同一 loop で起動する。
    event_bus = EventBus()
    ...
    try:
        ...
        with patch("src.core.engine.master_conductor_facade.ExecutionContext"), \
             ...
             patch("src.core.infra.event_bus.get_event_bus", return_value=event_bus), \
             patch("src.core.engine.master_conductor_bootstrap_coordinator.get_event_bus", return_value=event_bus), \
             patch("src.core.engine.master_conductor_facade.get_event_bus", return_value=event_bus), \
