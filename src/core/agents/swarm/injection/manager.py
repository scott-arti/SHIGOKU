...
class InjectionManagerAgent(BaseManagerAgent):
    ...
    def _mutate_schema_candidate_value(self, key: str, value: Any) -> Any:
        ...
        if isinstance(value, list):
            ...
            first = value[0]
            mutated = self._mutate_schema_candidate_value(token, first)
            if mutated is None:
                return None
            return [mutated]
    ...
    def _extract_mass_assignment_schema_candidates(
        self,
        *,
        ...
        for body in response_bodies:
            parsed = self._parse_json_dict(body)
            if not parsed:
                continue
            ...
            for container in containers:
                for key, value in container.items():
                    ...
                    if not name:
                        continue
                    lowered = name.lower()
                    if lowered in self.EXCLUDED_TESTED_PARAMS or lowered.startswith("__"):
                        continue
                    if name in candidates:
                        continue
                    if not any(token in lowered for token in risk_tokens):
                        continue
                    mutated = self._mutate_schema_candidate_value(name, value)
                    if mutated is None:
                        continue
    ...
    def _build_mass_assignment_variant_payload(self, probe_payload: Dict[str, Any], marker: str) -> Dict[str, Any]:
    ...
    async def _run_api_minimal_check(self, url: str, base_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        schema_probe_fields = self._extract_mass_assignment_schema_candidates(
        ...
        if probe_method:
            ...
            if probe_status in {200, 201, 202, 204} and reflection_hit:
                reflection_recheck_payload = self._build_mass_assignment_variant_payload(
                    probe_payload,
            ...
            if probe_status in {200, 201, 202, 204} and not reflection_hit:
                recheck_payload = self._build_mass_assignment_variant_payload(
                    probe_payload,
            ...
            if not mass_assignment_finding_emitted and has_auth_context and not initial_probe_used_auth_context:
                ...
                if auth_probe_status in {200, 201, 202, 204} and auth_reflection_hit:
                    auth_reflection_recheck_payload = self._build_mass_assignment_variant_payload(
                        auth_probe_payload,
                ...
                if auth_probe_status in {200, 201, 202, 204} and not auth_reflection_hit:
                    auth_recheck_payload = self._build_mass_assignment_variant_payload(
                        auth_probe_payload,
